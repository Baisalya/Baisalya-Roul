...
| DevDesk | `C:\Users\baish\StudioProjects\devdesk\docs\DEVDESK_VERIFICATION_RUNBOOK.md`; then `docs\release\RELEASE_RUNBOOK.md` and `docs\release\ANDROID_SIGNING.md` | Android command is in the verification runbook; Windows packager is `tool\release\package_windows_store_msix.ps1` | `C:\Users\baish\StudioProjects\devdesk\docs\DEVDESK_RELEASE_LOG.md` | Not from the current dirty worktree. Finish/commit the other feature work, rerun all checks, increment versions, then build a **free** AAB/MSIX. Paid products stay inactive. |
