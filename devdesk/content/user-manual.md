...
OKF means **Open Knowledge Format** in DevDesk. The verified implementation
checks supported OKF v0.2 Markdown bundle rules. It separates official required
rules from DevDesk warnings, advice, and extensions.

An OKF bundle is a folder of linked Markdown concepts. A concept is one file
with a `type` in frontmatter and a normal Markdown body.
...
Review tests before publishing.
```

### Create or inspect an OKF workspace

1. Create or open a normal workspace.
2. Place the intended Markdown under the configured knowledge roots.
3. Open **Developer tools > Structure and OKF**.
4. Review the project summary.
5. Fix **Required** findings first.
6. Review warnings and optional advice separately.
7. Choose **Manual**, **Review changes**, or bounded **Safe automatic** handling.

### Import existing Markdown

Use **Open folder**. DevDesk analyzes first. It does not need to rewrite every
file merely to inspect it. Existing wiki links can remain readable as an
extension; new managed relationships use standard Markdown links.

### Common mistakes

- Missing or malformed frontmatter on a concept.
- An empty `type`.
- Treating optional advice as a conformance failure.
- Assuming a source or verification event can be invented automatically.
- Running large changes without Git or another backup.

DevDesk never invents sources, human verification, lifecycle decisions,
attestation results, or trust conclusions. See
[Structure checks (OKF)](structured-knowledge-okf.html) and
